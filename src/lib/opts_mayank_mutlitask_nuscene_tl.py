...
class opts(object):
  def __init__(self):
    ...
    # self.parser.add_argument('--demo', default='/home/mayank_s/codebase/others/centernet/CenterTrack/videos/nuscenes_mini.mp4', help='path to image/ image folders/ video. or "webcam"')
    # self.parser.add_argument('--demo', default='/home/mayank_s/Desktop/centertrack/test', help='path to image/ image folders/ video. or "webcam"')
    ...
    # self.parser.add_argument('--demo', default='/home/mayank_s/Desktop/template/farm_1/saved_temp_img', help='path to image/ image folders/ video. or "webcam"')
    # self.parser.add_argument('--demo', default='/home/mayank_s/datasets/farminton/new_cropped_Demo/mayank_1', help='path to image/ image folders/ video. or "webcam"')
    self.parser.add_argument('--demo', default='/home/mayank_s/datasets/farminton/demo_route_complete_images/parking_lot_4', help='path to image/ image folders/ video. or "webcam"')
    # self.parser.add_argument('--demo', default='/home/mayank_s/datasets/farminton/demo_route_complete_images/avlanche', help='path to image/ image folders/ video. or "webcam"')
    # self.parser.add_argument('--demo', default='/home/mayank_s/Downloads/nuimages-v1.0-mini/sweeps/CAM_FRONT', help='path to image/ image folders/ video. or "webcam"')
    ...
    self.parser.add_argument('--save_point', type=str, default='5',
    ...
    self.parser.add_argument('--vis_gt_bev', default='True', help='')
    self.parser.add_argument('--kitti_split', default='3dop',
                             help='different validation split for kitti: '
                                  '3dop | subcnn')
    self.parser.add_argument('--test_focal_length', type=int, default=633)
    ...
    self.parser.add_argument('--track_thresh', type=float, default=0.03)
    self.parser.add_argument('--new_thresh', type=float, default=0.03)
    ...
    self.parser.add_argument('--hm_bdd_weight', type=float, default=1,
                             help='loss weight for keypoint heatmaps.')
    self.parser.add_argument('--off_bdd_weight', type=float, default=1,
                             help='loss weight for keypoint local offsets.')
    self.parser.add_argument('--wh_bdd_weight', type=float, default=0.1,
                             help='loss weight for bounding box size.')
  ...
  def update_dataset_info_and_set_heads(self, opt, dataset):
    ...
    weight_dict = {'hm': opt.hm_weight, 'wh': opt.wh_weight,
                   ...
                   'hm_bdd': opt.hm_bdd_weight,
                   'wh_bdd': opt.wh_bdd_weight,
                   'reg_bdd': opt.off_bdd_weight,
                   'hm_tl': opt.hm_bdd_weight,
                   'wh_tl': opt.wh_bdd_weight,
                   'reg_tl': opt.off_bdd_weight
                   }
  ...
  def init(self, args=''):
    ...
    default_dataset_info = {
      ...
      'tracking,ddd': 'nuscenes', 'tracking,ddd,multitask': 'BDD'
        ,'tracking,ddd,multitask_tl': 'BDD_tl'
    }
    opt = self.parse()
