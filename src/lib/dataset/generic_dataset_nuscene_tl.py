...
class GenericDataset(data.Dataset):
  ...
  def __init__(self, opt=None, split=None, ann_path=None, img_dir=None,ann_path_tl=None):
    ...
    if ann_path_tl is not None and img_dir is not None:
      print('==> initializing {} data from {}, \n images from {} ...'.format(split, ann_path_tl, img_dir))
      self.coco_tl = coco.COCO(ann_path_tl)
      self.images_tl = self.coco_tl.getImgIds()
      
      # if opt.tracking:
      #   if not ('videos' in self.coco_tl.dataset):
      #     self.fake_video_data()
      #   print('Creating video index!')
      #   self.video_to_images = defaultdict(list)
      #   for image in self.coco_tl.dataset['images']:
      #     self.video_to_images[image['video_id']].append(image)
      ########################################3333
      # search_image='n008-2018-07-26-12-13-50-0400__CAM_FRONT__1532621805662404.jpg'
      # for key, value in self.coco_tl.imgs.items():
      #   print('{}: {}'.format(key, value))
      #   if value['file_name']==search_image:
      #     print('found')
      #     index=(value['id'])
      #
      #     img, anns, img_info, img_path = self._load_data_2(index)
      #     print(img_path)
      ##########################################
  ...
  def __getitem__(self, index):
    ...
    ##################################################3333
    # search_image = 'n008-2018-07-26-12-13-50-0400__CAM_FRONT__1532621805662404.jpg'
    # search_image=img_path.split("/")[-1]
    # for key, value in self.coco_tl.imgs.items():
    #   # print('{}: {}'.format(key, value))
    #   if value['file_name'] == search_image:
    #     print('found')
    #     index = (value['id'])
    #     img_tl, anns_tl, img_info_tl, img_path_tl = self._load_data_2(index)
    #     print(img_path_tl)
    #     break
    ########################################################3
    ...
    if opt.tracking:
      pre_image, pre_anns, frame_dist = self._load_pre_data( img_info['video_id'], img_info['frame_id'], img_info['sensor_id'] if 'sensor_id' in img_info else 1)
      if flipped:
      ...
      ########################################################3
          ...
          if self.split == 'train' or self.split == 'val':
            ...
            if flipped:
              # flipped = 1
              # img = img[:, ::-1, :]
              anns_tl = self._flip_anns(anns_tl, width)
          ...
          # opt.tracking=False
          # if opt.tracking:
          #   pre_image, pre_anns, frame_dist = self._load_pre_data(img_info['video_id'], img_info['frame_id'],
          ...
          #
          # ### init samples
          # # self._init_ret_tl(ret, gt_det)
          # calib = self._get_calib(img_info_tl, width, height)

          num_objs = min(len(anns_tl), self.max_objs)
          for k in range(num_objs):
            ann = anns_tl[k]
            cls_id = int(self.cat_ids[ann['category_id']])
            if cls_id > self.opt.num_classes or cls_id <= -999:
            ...
            if cls_id <= 0 or ('iscrowd' in ann and ann['iscrowd'] > 0):
              self._mask_ignore_or_crowd(ret, cls_id, bbox)
              continue
            self._add_instance_tl( ret, gt_det, k, cls_id, bbox, bbox_amodal, ann, trans_output, aug_s, calib, pre_cts, track_ids)
            # self._add_instance( ret, gt_det, k, cls_id, bbox, bbox_amodal, ann, trans_output, aug_s, calib, pre_cts)
  ...
  def _load_image_anns_tl(self, img_id, coco, img_dir):
    img_info = coco.loadImgs(ids=[img_id])[0]
    file_name = "samples/CAM_FRONT/"+img_info['file_name']
    img_path = os.path.join(img_dir, file_name)
    ann_ids = coco.getAnnIds(imgIds=[img_id])
    anns = copy.deepcopy(coco.loadAnns(ids=ann_ids))
    img = cv2.imread(img_path)
  ...
  def _load_data_2(self, index):
    coco = self.coco_tl
    img_dir = self.img_dir
    img_id = self.images_tl[index]-1
    img, anns, img_info, img_path = self._load_image_anns_tl(img_id, coco, img_dir)
  ...
  def _init_ret_tl(self, ret, gt_det):
    max_objs = self.max_objs * self.opt.dense_reg
    ret['hm_tl'] = np.zeros( (1, self.opt.output_h, self.opt.output_w), np.float32)
    ret['ind_tl'] = np.zeros((max_objs), dtype=np.int64)
    ret['cat_tl'] = np.zeros((max_objs), dtype=np.int64)
    ret['mask_tl'] = np.zeros((max_objs), dtype=np.float32)

    regression_head_dims = {
      'reg_tl': 2, 'wh_tl': 2, 'tracking': 2, 'ltrb': 4, 'ltrb_amodal': 4, 
  ...
  def _init_ret(self, ret, gt_det):
    ...
    regression_head_dims = {
      'reg': 2, 'wh': 2,'reg_tl': 2, 'wh_tl': 2, 'tracking': 2, 'ltrb': 4, 'ltrb_amodal': 4, 
      'nuscenes_att': 8, 'velocity': 3, 'hps': self.num_joints * 2, 
      'dep': 1, 'dim': 3, 'amodel_offset': 2}
