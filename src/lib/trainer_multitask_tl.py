...
class GenericLoss(torch.nn.Module):
  ...
  def _sigmoid_output(self, output):
    ...
    if 'hm_bdd' in output:
      output['hm_bdd'] = _sigmoid(output['hm_bdd'])
    if 'hm_tl' in output:
    ...
    for s in range(opt.num_stacks):
      ...
      if 'hm_tl' in output:
        losses['hm_tl'] += self.crit(
          output['hm_tl'], batch['hm'], batch['ind'],
          batch['mask'], batch['cat']) / opt.num_stacks
      
      # regression_heads = ['reg', 'wh', 'tracking', 'ltrb', 'ltrb_amodal', 'hps', 'dep', 'dim', 'amodel_offset', 'velocity']
      regression_heads = ['reg_tl', 'wh_tl']
      ...
      for head in regression_heads:
        if head in output:
          losses[head] += self.crit_reg(
            output[head], batch[(head.split("_tl")[0]) + '_mask'],
            batch['ind'], batch[head.split("_tl")[0]]) / opt.num_stacks
...
class Trainer(object):
  ...
  def _get_losses(self, opt):
    # loss_order = ['hm_bdd', 'wh_bdd', 'reg_bdd','hm', 'wh', 'reg', 'ltrb', 'hps', 'hm_hp', \
    #   'hp_offset', 'dep', 'dim', 'rot', 'amodel_offset', \
    #   'ltrb_amodal', 'tracking', 'nuscenes_att', 'velocity']
    # loss_order = ['hm_bdd', 'wh_bdd', 'reg_bdd']
    loss_order = ['hm_tl', 'wh_tl', 'reg_tl']
