...
from ..generic_dataset_nuscene_tl import GenericDataset
from utils.ddd_utils import compute_box_3d, project_to_image

class nuScenes_tl(GenericDataset):
  ...
  def __init__(self, opt, split):
    ...
    if opt.dataset_version == 'test':
      ...
      ann_path = os.path.join(data_dir, 'annotations', '{}{}.json').format(opt.dataset_version, split_name)
      ann_path_tl=os.path.join(data_dir, 'annotations', '{}{}{}.json').format(opt.dataset_version, split_name,"_nuscenes_tl")
      # ann_path=os.path.join(data_dir, 'annotations', '{}{}{}.json').format(opt.dataset_version, split_name,"_nuscenes_tl")
    self.images = None
    super(nuScenes_tl, self).__init__(opt, split, ann_path, img_dir,ann_path_tl)
