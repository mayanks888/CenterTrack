...
for ind, da1 in enumerate(sorted(mygroup.keys())):
    ...
    for read_index in index:
        ...
        cv2.putText(image_scale, str(name[read_index]),(int((y[read_index][0] + y[read_index][2]) / 2), int(y[read_index][1] - 10)), cv2.FONT_HERSHEY_SIMPLEX, .5, (255, 255, 0), lineType=cv2.LINE_AA)
